...
export class ChamadoCreateComponent implements OnInit {
  ...
  create(): void {
    const payload = {
      ...
      // O JAVA EXIGE ESTES NOMES EXATOS:
      tecnicoId:   Number(this.tecnico.value),
      clienteId:   Number(this.cliente.value),
      
      // Enviamos estes também só por garantia (caso mude o Java), mas o importante são os de cima
      tecnico:     Number(this.tecnico.value),
      cliente:     Number(this.cliente.value)
    }

    this.chamadoService.create(payload as any).subscribe(resposta => {
